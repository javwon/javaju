...
//		e.print_arr(m3, ROW);
		...
		String[][] words = {{"computer","컴퓨터"},{"television","텔레비전"},{"phone","핸드폰"}};
		Scanner input2 = new Scanner(System.in);
		
		for(int i=0;i<words.length;i++)
		{ int j=0;
			 while(j<words[i].length-1)
				{System.out.println("Q"+(j+1)+words[i][j]+"의 뜻은? ");
					String input23 = input2.nextLine();
					j++;
					if(words[i][j].equals(input23)) System.out.println("정답입니다");
